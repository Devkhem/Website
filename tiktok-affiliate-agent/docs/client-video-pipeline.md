...
งานที่เคยอัดด้วย `--voice-id` จะจำเสียงนั้นไว้ใน `voiceover/settings.json`
และยึดค่านั้นต่อไป ไม่ถูกค่าใน `.env` มาทับ
...
ระบบจะ probe ไฟล์ด้วย ffprobe ว่าเปิดได้จริง ความยาวและสัดส่วนตรงกับ brief
ถ้าเครื่องยังไม่มี ffmpeg ให้ติดตั้งก่อน ไม่งั้นขั้นตัดต่อจะไม่ผ่าน
ถ้ามีหลายเวอร์ชันในนั้น ระบบจะใช้ไฟล์ที่แก้ล่าสุดเป็นตัวส่งรีวิว และบอกไว้ตอนรัน
